...
const DetalleEvaluaciones = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const fetchEvaluaciones = async () => {
      try {
        const response = await fetch(`http://192.168.100.45:3000/api/evaluaciones/paciente/${pacienteId}?kinesiologoId=${kinesiologoId}`);
        if (!response.ok) {
  ...
  const handleDelete = async (evaluacionId) => {
    try {
      const response = await fetch(`http://192.168.100.45:3000/api/evaluaciones/${evaluacionId}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
